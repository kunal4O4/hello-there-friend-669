...
import TimelineNode from "./portfolio/TimelineNode";
  ...
  return (
    <section id="portfolio" className="py-24 bg-gradient-subtle">
      <div className="container mx-auto px-4">
        ...
        <h3 className="text-2xl font-bold text-center mb-16">Professional Journey</h3>
        <div className="max-w-6xl mx-auto relative min-h-[3000px]">
          {/* SVG Timeline Path */}
          <svg 
          ...
          >
            ...
            <path
              d={experiences.map((_, i) => {
                const spacing = 300 / experiences.length;
                const y = i * spacing + spacing / 2;
                const x = 50;
                const controlX = i % 2 === 0 ? 35 : 65;
                const prevY = i === 0 ? 0 : (i - 1) * spacing + spacing / 2;
                const midY = (prevY + y) / 2;
                
                if (i === 0) return `M ${x} ${y}`;
                return `C ${controlX} ${prevY + (y - prevY) * 0.3}, ${controlX} ${prevY + (y - prevY) * 0.7}, ${x} ${y}`;
              }).join(' ')}
